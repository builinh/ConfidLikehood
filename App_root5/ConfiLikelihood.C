...
double fwidth=gClient->GetDisplayWidth()*0.9;
double fhight=gClient->GetDisplayHeight()*0.8;
...
void ConfidenceLikelihood::AddCanvas() 
{
    ...
    if(IsReset){   lCanvases.Delete();}
	  for(Int_t numberC = 0; numberC<fNumberOfPeaks; numberC++){
  	  ...
  	  fEcan = new TRootEmbeddedCanvas (tmp, fVer, fwidth, fhight);
...
// Compute the limits
  ...
  myfile << "**********************************************"<<endl;
  myfile << "----------------------------------------------"<<endl;
  myfile << "The local date and time run is: "<<dt<<endl;
  myfile << "Input file: "<< fi.fFilename<<endl;
  myfile << "Confidence level test for peak "<<i<<endl;
  myfile << "Computing limits... " <<endl;
  myfile << "CLs    : "   << myconfidence->CLs()  << endl;
  myfile << "CLsb   : "   << myconfidence->CLsb() << endl;
  myfile << "CLb    : "   << myconfidence->CLb()  << endl;
  myfile << "< CLs >  : " << myconfidence->GetExpectedCLs_b()  <<endl;
  myfile << "< CLsb > : " << myconfidence->GetExpectedCLsb_b() <<endl;
  myfile << "< CLb >  : " << myconfidence->GetExpectedCLb_b()  <<endl;
